...
WHERE id = $1
RETURNING id, organization_id, event_id, category_id, order_id, participant_id, ticket_number, status, holder_name, holder_email, event_title, category_name, qr_version, issued_at, used_at, created_at, updated_at, bib_number, bib_assigned_at, bib_assigned_by, bib_assignment_method, wave_id, form_answers
`
...
const linkTicketToParticipant = `-- name: LinkTicketToParticipant :one
UPDATE tickets
...
type LinkTicketToParticipantParams struct {
	OrderID       uuid.UUID
	ParticipantID *uuid.UUID
}

func (q *Queries) LinkTicketToParticipant(ctx context.Context, arg LinkTicketToParticipantParams) (Ticket, error) {
	row := q.db.QueryRow(ctx, linkTicketToParticipant, arg.OrderID, arg.ParticipantID)
	var i Ticket
	err := row.Scan(
		&i.ID,
		&i.OrganizationID,
...
SELECT id, organization_id, event_id, category_id, order_id, participant_id, ticket_number, status, holder_name, holder_email, event_title, category_name, qr_version, issued_at, used_at, created_at, updated_at, bib_number, bib_assigned_at, bib_assigned_by, bib_assignment_method, wave_id, form_answers FROM tickets
...
const listTicketsForBroadcast = `-- name: ListTicketsForBroadcast :many
...
type ListTicketsForBroadcastParams struct {
	OrganizationID uuid.UUID
	EventID        uuid.UUID
	CategoryID     *uuid.UUID
}

type ListTicketsForBroadcastRow struct {
	HolderName   string
	HolderEmail  string
	TicketNumber string
	CategoryName string
...
func (q *Queries) ListTicketsForBroadcast(ctx context.Context, arg ListTicketsForBroadcastParams) ([]ListTicketsForBroadcastRow, error) {
	rows, err := q.db.Query(ctx, listTicketsForBroadcast, arg.OrganizationID, arg.EventID, arg.CategoryID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListTicketsForBroadcastRow
	for rows.Next() {
		var i ListTicketsForBroadcastRow
		if err := rows.Scan(
			&i.HolderName,
			&i.HolderEmail,
			&i.TicketNumber,
...
const updateTicketParticipant = `-- name: UpdateTicketParticipant :one
UPDATE tickets SET
    holder_name = $2,
    holder_email = $3,
    form_answers = COALESCE($6, form_answers),
    updated_at = now()
WHERE id = $1 AND organization_id = $4 AND event_id = $5
RETURNING id, organization_id, event_id, category_id, order_id, participant_id, ticket_number, status, holder_name, holder_email, event_title, category_name, qr_version, issued_at, used_at, created_at, updated_at, bib_number, bib_assigned_at, bib_assigned_by, bib_assignment_method, wave_id, form_answers
`

type UpdateTicketParticipantParams struct {
	ID             uuid.UUID
	HolderName     string
	HolderEmail    string
	OrganizationID uuid.UUID
...
func (q *Queries) UpdateTicketParticipant(ctx context.Context, arg UpdateTicketParticipantParams) (Ticket, error) {
	row := q.db.QueryRow(ctx, updateTicketParticipant,
		arg.ID,
		arg.HolderName,
		arg.HolderEmail,
		arg.OrganizationID,
