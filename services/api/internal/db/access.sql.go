...
const consumeGrant = `-- name: ConsumeGrant :one
...
func (q *Queries) ConsumeGrant(ctx context.Context, arg ConsumeGrantParams) (AccessGrant, error) {
	row := q.db.QueryRow(ctx, consumeGrant, arg.ID, arg.OrderID)
	var i AccessGrant
	err := row.Scan(
		&i.ID,
		&i.PoolID,
